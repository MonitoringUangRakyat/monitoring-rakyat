...
function ensureGudangPanel() {
  ...
  const target =
    document.getElementById("page-ai-settings") ||
...
function renderOrchestratorStatus(status) {
  ...
  const uploadPolicy = status.public_db_upload_policy || {};
  const uploadThresholds = uploadPolicy.triage_thresholds || {};
  const publicQueue = status.public_submission_queue || {};
  ...
  return `
    <div style="margin-top:12px;border:1px solid #f59e0b66;border-radius:8px;padding:10px;background:#1f1304">
      ...
      <p style="margin:8px 0 0;color:#cbd5e1;font-size:11px;line-height:1.5">
        ...
        Upload rakyat pending audit: ${publicQueue.count ?? 0}.
        Status: ${statusText}.
      </p>
      ${controller.id ? `
      ...
      ${uploadPolicy.entry_status ? `
        <p style="margin:8px 0 0;color:#fef3c7;font-size:11px;line-height:1.5">
          Upload DB rakyat: ${(uploadPolicy.accepted_inputs || []).join(", ")}.
          Valid >= ${uploadThresholds.minimum_queue_score || 50}% masuk audit queue; di bawah ${uploadThresholds.reject_below_score || 40}% audit ketat/fake kick.
        </p>` : ""}
      ${top.length ? `
        <div style="display:grid;gap:6px;margin-top:8px">
          ${top.map((item) => `
...
function renderFiscalRatioChart(payload) {
  ...
  const maxVal = Math.max(
    ...rows.map((row) => Math.max(
      ...
      Number(row.kerugian_korupsi_t || 0)
    )),
    1
  );
  ...
  chart.innerHTML = rows.map((row) => {
    ...
    const korupsi = Number(row.kerugian_korupsi_t || 0);
    const korupsiRatio = Number(row.korupsi_vs_belanja_pct || 0);
    const isEmpty = !apbn && !pajak && !sda && !korupsi;
    const valueLabel = (value) => value > 0 ? `<span class="cmp-val">${formatTrillion(value)}</span>` : "";
    return `
      <div class="cmp-group" title="Sinkron Gudang DB ${year}: ${row.status || "UNKNOWN"} | Kerugian korupsi vs belanja: ${korupsiRatio}%">
        ...
        <div class="cmp-bars">
          ...
          <div class="cmp-bar b-korupsi" title="Kerugian Korupsi ${year}: ${korupsi ? formatTrillion(korupsi) : "Kosong"} Rp T (${korupsiRatio}% dari Belanja APBN)" style="height:${barHeight(korupsi)}px;opacity:${korupsi ? 1 : .28}">${valueLabel(korupsi)}</div>
        </div>
  ...
  if (desc && !desc.dataset.fiscalRatioSync) {
    ...
    desc.innerHTML += `<br><span style="color:#7dd3fc">Sinkron Gudang DB: ${payload.data.length} baris agregasi tahunan. Termasuk kerugian korupsi per tahun untuk membaca ratio terhadap APBN/Pajak/SDA.</span>`;
  }
  return true;
}
