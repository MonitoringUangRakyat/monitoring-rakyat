...
REQUIRED = [
    ...
    "dashboard/public_submission_status.json",
    ...
    "gudang-db/_queue/public_submissions.csv",
    ...
    "scripts/import_public_submissions.py",
    "scripts/promote_public_submissions.py",
    "scripts/promote_ai_hunter_candidates.py",
    "scripts/sanitize_public_submission_refs.py",
    "scripts/seed_fiscal_baseline.py",
...
def main() -> int:
    ...
    if run([sys.executable, "scripts/validate_immutable_db.py"]) != 0:
        return 1
    if run([sys.executable, "scripts/import_public_submissions.py"]) != 0:
        return 1
    if run([sys.executable, "scripts/promote_public_submissions.py"]) != 0:
        return 1
    ...
    if run([sys.executable, "scripts/ai_pengawas_orchestrator.py"]) != 0:
        return 1
    if run([sys.executable, "scripts/promote_ai_hunter_candidates.py"]) != 0:
        return 1
    if run([sys.executable, "scripts/sanitize_public_submission_refs.py"]) != 0:
        return 1
    if run([sys.executable, "scripts/generate_ai_index.py"]) != 0:
        return 1
    if run([sys.executable, "scripts/seed_fiscal_baseline.py"]) != 0:
        return 1
