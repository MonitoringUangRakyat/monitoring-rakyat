...
- terima format fleksibel: link, Excel, Word, PDF, catatan/noted, CSV, atau teks biasa;
...
- beri skor validitas awal 0-100;
- jika skor `>= 50`, masukkan ke audit queue Gudang DB sesegera mungkin untuk dianalisa dan dimapping ke halaman/menu terkait;
- jika skor `40-49`, simpan sebagai task pencarian pembanding dan jangan jadikan klaim publik;
- jika skor `< 40`, audit link/dokumen/sumber secara ketat;
- tandai `NEEDS_MORE_EVIDENCE` jika sumber kurang;
- tandai `REJECTED_NO_SOURCE` jika tidak ada sumber;
- tandai `REJECTED_FAKE_OR_UNVERIFIABLE` jika fake, spam, fitnah, doxing, atau tidak bisa dicek;
...
Hardcode keras: data base halaman harus bisa tampil sesegera mungkin setelah kontribusi lolos audit minimal. Setiap data baru yang lolos queue wajib memicu regenerate index, dashboard summary, fiscal ratio bila relevan, readiness, source patrol, orchestrator status, dan sinkronisasi menu.
